...
class EditEducationItemPage extends Component {
  state = {
    formSubmitted: false,
    formIsValid: false,
    form: {
  ...
  async getData() {
    ...
    form.fieldOfStudyId.value = educationItemData.fieldOfStudyId;
    ...
    form.specializationId.elementConfig.options = allSpecializations;
    form.specializationId.value = educationItemData.specializationId;

    form.currentSemester.value = educationItemData.currentSemester;
    form.currentAcademicYear.value = educationItemData.currentAcademicYear;
  ...
  handleSubmit = (event) => {
    ...
    axios.patch(
      `http://localhost:4000/education/${this.props.match.params.educationItemId}`,
      formData,
      {
        headers: {
          Authorization: `Bearer ${this.props.token}`,
  ...
  render() {
    if (this.state.formSubmitted) {
      return (
        <Redirect to={`/admin/users/${this.props.match.params.userId}/education/${this.props.match.params.educationItemId}`} />
      );
    }
    ...
    return (
      <Fragment>
        <PageHeader>
          <PageTitle>Edytuj informacje o ścieżce edukacji</PageTitle>
          <PageSubtitle>Identyfikator {this.props.match.params.educationItemId}</PageSubtitle>
        </PageHeader>
        <Container>
...
export default connect(mapStateToProps)(EditEducationItemPage);
