...
class EditResidenceAddressPage extends Component {
  state = {
    formSubmitted: false,
    formIsValid: false,
    form: {
  ...
  async getData() {
    ...
    if (userData.residenceAddress) {
      ...
      form.voivodeship.value = userData.residenceAddress.voivodeship;
      form.city.value = userData.residenceAddress.city;
      form.street.value = userData.residenceAddress.street;
      form.zipCode.value = userData.residenceAddress.zipCode;
      form.postOffice.value = userData.residenceAddress.postOffice;
      form.phoneNumer.value = userData.residenceAddress.phoneNumer;
  ...
  handleSubmit = (event) => {
    ...
    axios.patch(
      `http://localhost:4000/users/${this.props.match.params.userId}/address/residence`,
      formData,
      {
        headers: {
          Authorization: `Bearer ${this.props.token}`,
  ...
  render() {
    if (this.state.formSubmitted) {
      return (
        <Redirect to={`/admin/users/${this.props.match.params.userId}/address/residence`} />
      );
    }
    ...
    return (
      <Fragment>
        <PageHeader>
          <PageTitle>Edytuj informacje o adresie zamieszkania użytkownika</PageTitle>
...
export default connect(mapStateToProps)(EditResidenceAddressPage);
